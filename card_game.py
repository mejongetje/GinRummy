...
class CardGame:
    ...
    def __init__(self):
        ...
        self.p1 = Player('Human Player', human=True)
...
class GamePlay(GinRummy):
    def __init__(self, hands):
        ...
        self.rest_of_deck = self.board.playdeck[1:]
        self.discard = self.board.playdeck[0]
        self.turn = 0
    ...
    def adding_card(self) -> None:
        ...
        if self.active_player.human:
            ...
            add_discard = eval_discard(self.active_player, self.discard, self.discarded_cards)
        ...
        if add_discard:
            ...
            if self.active_player.human:
                print(f"{self.active_player} adds {card_from_deck} from the deck to his hand")
            else:
    ...
    def dropping_card(self):
        ...
        if self.active_player.human:
            ...
            add_status_to_deadwood_cards(self.active_player.hand.deadwood, self.discarded_cards, self.turn)
            dropped_card = find_discard_card(self.discard, self.active_player.hand.deadwood, self.active_player)
    ...
    def display_result(self):
        ...
        self.display_game()
        print("\nWE HAVE A WINNER!!!")
        active_deadwood_sum = sum([card.value for card in self.active_player.hand.deadwood])
        other_deadwood_sum = sum([card.value for card in self.other_player.hand.deadwood])
        winner = self.active_player if active_deadwood_sum < other_deadwood_sum else self.other_player
        points = other_deadwood_sum - active_deadwood_sum if winner == self.active_player \
